...
> 历史状态：本记录已由 [V2 验收记录](../calendar-v2/acceptance.md) 取代，仅保留 V1 当时的证据与结论，不回写为 V2 验收结果。
...
| `light-980x680.png` | `e4b9fb7` 视觉基线：当时权威 ZIP 解出的 App，浅色最小窗口、同一完整 fixture、无裁切 | 已重采并目检：实际 980×768；分类色和 `工/生/健/未`、完整 `09:00`、完整“产品同步会”、“还有 2 项”及六周月格均可读 |
| `dark-980x680.png` | `e4b9fb7` 视觉基线：当时权威 ZIP 解出的 App，深色最小窗口、同一完整 fixture、无裁切 | 已重采并目检：实际 980×768；与浅色使用同一 fixture，分类语义、完整时间和标题、“还有 2 项”均未丢失 |
