...
- [x] **Task 7B Wiring:** implement AppShell with `HStack(spacing: 0)`, a fixed 64pt leading Rail and CalendarModuleView retaining exactly the existing 980pt content minimum. Draw the divider inside the Rail so it adds no layout width. AppShell/window min width is 1044 and min height remains 680; keep the existing window ID, default size, Store load task and appearance binding, and set `.windowResizability(.contentMinSize)`. Move the current 1044 frame off MonthView to the shell boundary.
...
- [x] Wrap MonthView without changing internal layout, gestures, continuous scrolling, DayDrawer, create/edit overlay or category sheet. Route state owns only current route; each module owns its own selection and scroll state.
- [x] In Task 7B, AppShell retains one stable host lifetime for every enabled module and switches visibility only; hidden hosts disable hit testing and are accessibility-hidden. Feature-disabled module builders are invoked zero times and no `EmptyView`/placeholder page may satisfy route exhaustiveness. Use injectable sentinel modules to prove object identity, selection and scroll tokens survive route round trips, and explicitly test inactive-host interaction/accessibility flags; repeat with real Calendar↔Notes in Task 10.
- [x] Rail descriptors use Chinese help/VoiceOver names 日历、笔记、灵感. Selected state uses `theme.rangePreviewFill` in a rounded tile plus selected trait/non-color accessibility value; Rail background uses `theme.elevatedSurface`; inactive icons use `theme.secondaryText`. Only enabled route buttons are constructed.
- [x] Run GREEN plus focused interaction regressions, full CalendarAppTests and release build. Confirm static uniqueness: only AppEnvironment constructs WorkspaceStore, only WorkspaceCommands owns 1/2/3/N and DayDrawer/Rail own no duplicate shortcuts.
...
- [x] Define the complete command, environment, result, clipboard and typed-error interfaces before production behavior. `BlockIDSource.fixed` is consumed deterministically inside one reduce call; every inserted or fallback Block receives the next ID. Exhausted IDs, duplicate supplied IDs or collision with an existing Block ID throw before publishing any candidate. Never call `BlockDocument.empty()` from the reducer.
...
- [x] Define Task 8's AppKit-free paste and clipboard values. Payloads never contain Block IDs, fonts, colors, paragraph styles or completion timestamps. Task 9 converts `NSAttributedString` into these values; unsupported attributes lose styling only, never characters. A pasted task always starts with `completedAt == nil`.
...
- [x] Run the complete RED suite and record behavior failures, not test-fixture compilation mistakes. Do not create production files until the RED command fails only because the Task 8 APIs/behavior are absent.
...
- [x] Request fresh Sol xhigh review focused on selection direction, grapheme safety, exhaustive kind tables, ID/validator atomicity, inline span preservation, paste fidelity, stable-ID drag, undo directives and Task 9 bridge readiness. Fix every Critical/Important finding and rerun the full Task 8 gates.
...
- [x] Do not begin Task 9 until Task 8 is reviewed, committed and the following preflight is GREEN. Task 9 must consume the committed reducer rather than adding editor-only mutation algorithms.
...
- [x] Lock the only legal `BlockInputResult` consumption combinations and RED-test every row plus invalid combinations:
...
- [x] Add the complete public-shaped internal API skeleton and production composition changes, then run `swift build --target CalendarApp` before filling delegate behavior. Do not weaken MainActor isolation or use `@preconcurrency` to hide an unsafe editor path.

- [x] Implement the minimum production Session, bridge, text hosts, selection, pasteboard, slash, drag, focus-command routing and accessibility behavior that turns every RED matrix GREEN. The old `MarkdownNotesEditor` and `MarkdownRichTextCodec` remain untouched and serve legacy calendar notes only.

- [x] Run GREEN plus Task 8, command routing, CalendarApp, full and release gates. Verify only one PersonalCalendarApp focus registry, one editor session/UndoManager per editSessionID and no AppKit mutation bypass around `BlockInputReducer`.
...
- [x] Preserve `submitDraft(_:)` as a compatibility wrapper, but make the authoritative path two-stage and Store-owned. UI code never receives a Journal repository and never performs a sidecar write itself. Lock internal public-shaped contracts with semantics equivalent to:
...
- [x] Request fresh Sol xhigh review of Task 10A, fix every Critical/Important finding, explicitly stage only the 10A file list, verify the cached file list and commit it before creating 10B tests.
...
- [ ] Debounce from the latest edit timestamp: at 150ms protect only the latest generation; at 650ms total commit that exact protected token. A newer edit cancels only timer sleeps that have not fired. For each exact `(identity, generation, noteSnapshotChecksum)`, timer and flush callers share one coordinator-owned protect/commit operation; Store I/O that has started is never cancelled or replayed, and a Store-issued capability is consumed at most once. `flushLatest` is one MainActor-linearized barrier shared by selection change, route change, app inactive, window close and termination. It has two explicit stages: first enter `.finalizingNativeInput(permit)` and invoke the injected 10B finalizer contract. During that stage ordinary edits and stale hosts are rejected, while exactly one authoritative callback carrying the exact `(nonce, noteID, editSessionID, activeHostToken)` permit may commit the current marked candidate and synchronously increment generation; the permit is consumed once. If finalization succeeds (including no pending candidate), enter sealed read-only state, capture the exact triple, then reuse or start that triple's single protect/commit operation. A defensive generation/checksum change loops only after the in-flight operation reaches a truthful terminal. `commitPending` and cleanup-pending preserve the exact transaction/cleanup identity, block every successor operation and keep all callers waiting until explicit retry resolves them; only then may the latest newer triple start once. Concurrent timer/inactive/close/route/selection callers await the same operation/barrier and receive its original transaction identity. Finalization failure captures/persists nothing and returns to the truthful editable state.
- [ ] Lock a complete `NoteAutosaveState` mapping for `.committed`, `.noChange`, `.conflict`, `.draftSuperseded`, `.commitPending`, `.notCommitted`, `.externalSourceChanged`, `.persistenceBlocked` and Journal cleanup-pending. Its barrier evidence is typed as exact `persisted(triple)`, `protectedOnly(triple)` or `unsafe/latestUnprotected`. A definite main failure after exact unbind stays editable and says `保存失败—草稿已保护`; commit-uncertain and cleanup-pending are visibly read-only until their exact retry completes; protection failure says the draft is not protected and blocks silent close. Nil/wrong persisted-draft receipts, wrong identity/session/generation/checksum, and `.committed(.restore)` never count as a saved Note. No non-throw outcome is silently promoted to saved.
- [ ] `NoteCloseProtectionBridge` owns the platform-independent lifecycle decision seam that Task 10C will connect to the real window/application. It returns false/terminate-later while the shared linearized flush is pending. Selection change, route change and archive require exact `persisted`; app inactive may settle at exact `protectedOnly`; window close and termination may release at exact `protectedOnly` or `persisted`. Journal/main dual failure keeps the window open until explicit copy/export succeeds for the exact then-current triple and verified snapshot checksum. A copy of N, a failed copy, or any result for N never releases N+1. `.onDisappear` alone is not accepted as close protection.
...
- [ ] Create/archive/restore/permanent-delete are queued `WorkspaceStore.sendWorkspace` commands. Selection itself is a MainActor ViewModel state transition after the old Note's exact persisted flush succeeds; it is not a Workspace command and adds no new Domain case. Create direct success or retry-finally-committed selects the new Note exactly once; failure or pending leaves selection unchanged. Archiving first requires the latest draft to be committed to the main file—not merely Journal-protected—then sends exactly one `.archiveNote`; restore sends exactly one `.restoreNote`. Permanent-delete sends exactly one authorized preview command; stale authorization, failure or pending leaves selection unchanged. Direct or retry-finally-committed archive/restore/permanent-delete runs its fallback exactly once, using the item now occupying the same displayed index or the preceding item and showing a real empty state when none remain. Pending/cleanup never runs a fallback early. A protected-only/main-failed draft does not archive. Category definitions always come from the shared Workspace calendar categories; category edits on a Note are Journal-protected draft fields.
- [ ] Define the native-input finalizer as an injected 10B contract/closure using only `noteID`, `editSessionID`, active host token and an opaque nonce generated and consumed by the MainActor `NoteAutosaveCoordinator` barrier owner. This nonce authorizes only one current-editor native callback; it is not WorkspaceStore/Journal authority and never crosses into Persistence. 10B must compile and test without any AppKit/Task 9 production type. The 10C editor supplies the real implementation. Write public RED first so all three target suites initially fail on missing production contracts. Scheduler/permit edge tables may use deterministic fakes, but protect→commit, Journal-only main failure, dual failure, commitPending, cleanup-pending and exact retry must also pass through a real `WorkspaceStore` with exactly one temporary `DraftJournalRepository`; no ViewModel/coordinator/bridge may create or retain another repository. Cover suspended 150ms protect plus simultaneous 650ms/close, suspended 650ms commit plus concurrent route/close/selection, N→pending/cleanup with N+1 blocked until exact retry, original transaction IDs, 149/150ms and 649/650ms, continuous generations, exact permitted terminal callback accepted once, duplicate/ordinary/stale-host callbacks rejected, edit-during-await, exact latest-triple comparison, caller-specific persisted/protected release, selection zero Workspace commands, create/archive/restore/permanent-delete each exactly one command, direct success selection, pending→retry committed fallback exactly once, failure/stale authorization selection stability, permanent-delete same-index/previous/empty fallback, Journal/main dual failure, exact latest copy/export release, Chinese title/body search, category intersection, exact UUID-string tie break, and recent/all/archive. Exhaustive pure outcome fixtures remain supplemental and cannot replace these real Store integration REDs.
